...
└── src/
    ├── WebApp.sln                  # Решение (API + тесты + фронтенд)
    ├── backend/
    │   ├── WebApp.Api/             # Backend
    ...
    │   │   └── WebApp.Api.csproj
    │   └── WebApp.Api.Tests/       # Модульные тесты (xUnit)
    │       └── Services/
    │           ├── SumNumbersServiceTests.cs  # Пример тестов
    │           └── _TEMPLATE.cs              # Шаблон для новых тестов
    ├── frontend/
...
### 4. Написать тесты на утилиту

```csharp
// Создать Services/MyNewServiceTests.cs в проекте WebApp.Api.Tests
public class MyNewServiceTests
{
    ...
    {
        var result = _service.Execute("тестовые данные");
...
### 5. (Опционально) Добавить UI во frontend
...
dotnet test --filter "Category=Example"  # только примеры
```
